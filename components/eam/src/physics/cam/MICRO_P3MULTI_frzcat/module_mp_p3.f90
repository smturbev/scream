...
if (depo_frz_std) then !BG
  ...
  if ( ( (t(i,k).lt.258.15) .and. (t(i,k) .ge. 236.15) .and. (supi(i,k).ge.0.05) .and. (qc(i,k) .gt. qsmall) ) .or. ( (t(i,k).lt.241.15) .and. (t(i,k) .ge. 236.15) .and. (supi(i,k).ge.0.005) .and. (qc(i,k) .gt. qsmall) ) ) then  !BG added this to prevent too much "hom frz at -37"
...
endif !ice nucleation BG or standard P3
...
! homogeneous freezing of cloud and rain
    ...
    ! compute mean-mass ice diameters (estimated; rigorous approach to be implemented later)
      ...
      if (do_accumulate_micro_proc_rates) then
         ...
         idx=idx+1   !number 23 and 15 are nucleation QI
...
!-------------------------------------------
      ...
      if (i_type.EQ.1 .and. T.lt.273.15 .AND. (.NOT.useLiquidESat) ) then
        ! ICE

        if(useMurphyKoopESat) then
          !bloss(2019-03-27): p_ice from Murphy and Koop (2005, QJ, https://doi.org/10.1256/qj.04.94, Eqn 7)
          ! Temperature T in K and result in Pa.  This is valid at lower temperatures
          ! and might be worth using in simulations that seek to represent the TTL
          ! well or that have a very cold tropopause.
          POLYSVP1 = exp( 9.550426 - 5723.265/T + 3.53068*log(T) - 0.00728332*T )
      ...
      else !bloss: Always produce a saturation vapor pressure!  if (i_type.EQ.0 .or. T.ge.273.15) then

        ! LIQUID
        if(useMurphyKoopESat) then
          !bloss(2019-03-27): p_liq from Murphy-Koop (2005, QJ, equation 10):
          POLYSVP1 = EXP( &
               54.842763 - 6763.22/T - 4.210*log(T) + 0.000367*T &
               + tanh( 0.0415*(T-218.8) ) * ( 53.878 - 1331.22/T - 9.44523*log(T) + 0.014025*T ) &
               )
        else
          !       Flatau formulation: (Note that esat = constant below -80C. - bloss)
          dt       = max(-80.,t-273.16)
